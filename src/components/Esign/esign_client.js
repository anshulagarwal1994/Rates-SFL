...
class esign_client extends React.Component {
  ...
  async componentDidMount() {
    this.setState({hideIntital : this.props.location.pathname.includes('/false')});
      ...
      api
        ...
        .then(async (res) => {
          if (res.success) {
            ...
            if (res.data) {
              const dateLimit = moment(res.data.EndDate, 'YYYY-MM-DDTHH:mm:ss.SSSZ');
              const now = moment()
  ...
  setdata(lable, value) {
    ...
    if (lable == 'Inclusions') {
      ...
      if (value.length > 1) {
        ...
        value.forEach(element => {
          var tempdata = { value: element.Inclusions, label: element.Inclusions };
    ...
    if (lable == 'Exclusions') {
      if (value.length > 1) {
        ...
        value.forEach(element => {
          var tempdata = { value: element.Exclusions, label: element.Exclusions };
          arrayTemp.push(tempdata);
        });
    ...
    if (
      ...
      for (var i = 0; i < PackageList.length; i++) {
        if (PackageList[i].Status === "Active") {
          ...
          TotalInsuredvalue = Number(TotalInsuredvalue) + Number(PackageList[i].InsuredValue);
  ...
  renderInvoice = () => {
    return this.state.invoiceList.map((invoice, idx) => {
      return (
        <tr>
          <td data-label="" style={{ border: '1px solid #000', padding: '3px', width: '20px', textAlign: 'center' }}>{idx + 1}.</td>
          <td data-label="DESCRIPTION OF SERVICES" style={{ border: '1px solid #000', padding: '3px' }}>{invoice.ServiceDescription}{invoice.Description ? " - " + invoice.Description : ""}</td>
          <td data-label="COST (IN USD)" style={{ border: '1px solid #000', padding: '3px', textAlign: 'right', width: '170px' }}>{invoice.TotalAmount ? "$" : null}{invoice.TotalAmount ? parseFloat(invoice.TotalAmount).toFixed(2) : null}</td>
        </tr>)
    });
  }
  ...
  validate(){
    ...
    if(CommonConfig.isEmpty(this.state.initialHere[4]["5"]) && this.state.isVisible === true){
      IsFormValid = false;
      this.scrolldiv(1.5);
      return IsFormValid;
    }
  ...
  trim = () => {
    if(this.validate()){
      ...
      try {
        ...
        api.post("scheduleshipment/GeneratedContract", data)
          .then((res) => {
            this.hideLoader();
            if (res.success) {
  ...
  scrolldiv(pageNumber) { 
    window.scrollTo(0,this.findPosition(document.getElementById("footer-"+pageNumber))); 
  } 
  ...
  footerHtml = (pageNumber) => {
    return (
      <table className="footer-esign table">
          <tr>
            <td id={"footer-"+pageNumber} className="relative">Initial Here: 
              <input disabled={this.state.hideIntital} onChange={(e)=>this.intialHereChange(e,pageNumber)} maxlength="3"/>
              ...
              {pageNumber < 4 ? 
                pageNumber === 1 && this.state.isVisible === true ?  
                  <div className="flag-label">
                    <span onClick={() => this.scrolldiv(pageNumber+0.5)}>Sign</span>
                  </div>
                  :
                  <div className="flag-label">
                    <span onClick={() => this.scrolldiv(pageNumber+1)}>Sign</span>
                  </div>
                :
                <div className="flag-label">
                  <span>Sign</span>
            ...
            <td className="footer-esign center"><b>SFL WORLDWIDE LLC | FMC License No.: 025184</b></td>
            <td className="footer-esign right">Page <b>{pageNumber}</b> of <b>4</b></td>
          </tr>
        </table>
    )
  }
  ...
  render() {
    ...
    return (
      <div className="esign-page-table">
        ...
        {this.state.ShowContra ? (<div>
          <div className="page-break">
            {this.headerHtml()}
              <div class="table-pane active">
              <table style={{ width: '100%', marginTop: "15px" }}>
                <tr>
                  <td style={{ border: '1px solid #000', textAlign: 'center', padding: '3px', fontSize: '14px', fontWeight: 'bold', 'color': '#fff', background: "#002060" }}>Contract for Overseas Relocation Services (LCL)</td>
                </tr>
              </table>
                <table style={{ width: "100%" }}>
                  <tr>
                    <td style={{ width: "50%", padding: '0px'}} className="td-mobile-full">
                      <table style={{ width: "100%", tableLayout: 'fixed' }}>
                        <tr>
                          <td style={{ border: '1px solid #000', width: '20%', padding: '3px' }}><b>Customer Name</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}>{this.state.FromContactName}</td>
                        </tr>
                        <tr>
                          <td style={{ border: '1px solid #000', width: '20%', padding: '3px' }}><b>Origin City:</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}>
                            {this.state.fromCity}, {this.state.fromStateShow} - {this.state.FromZipCode} , {this.state.selectedFromCountry.label}
                          </td>
                        </tr>
                        <tr>
                          <td style={{ border: '1px solid #000', width: '20%', padding: '3px' }}><b>Phone:</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}>{this.state.FromPhone1}{!CommonConfig.isEmpty(this.state.FromPhone2) ? "," : ""}{this.state.FromPhone2}</td>
                        </tr>
                        <tr>
                          <td style={{ border: '1px solid #000', width: '20%', padding: '3px' }}><b>Est. Volume</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}>{this.state.totalCFT}</td>
                        </tr>
                        <tr>
                          <td style={{ border: '1px solid #000', width: '20%', padding: '3px' }}><b>Commodity</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}>HHG</td>
                        </tr>
                        <tr>
                          <td style={{ border: '1px solid #000', width: '20%', padding: '3px' }}><b>Service</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}>{this.state.SubServiceName.value}</td>
                        </tr>
                      </table>
                    </td>
                    <td style={{ width: "50%", padding: '0px'}} className="td-mobile-full">
                      <table style={{ width: "100%", tableLayout: 'fixed' }}>
                        <tr>
                          <td className="mobile-align-left" style={{ border: '1px solid #000', width: '20%', padding: '3px', textAlign: 'right' }}><b>Quote Ref.:</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}>{this.state.TrackingNumber}</td>
                        </tr>
                        <tr>
                          <td className="mobile-align-left" style={{ border: '1px solid #000', width: '20%', padding: '3px', textAlign: 'right' }}><b>Delivery City:</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}>
                            {this.state.toCity}, {this.state.toStateShow} - {this.state.ToZipCode} , {this.state.selectedToCountry.label}
                          </td>
                        </tr>
                        <tr>
                          <td className="mobile-align-left" style={{ border: '1px solid #000', width: '20%', padding: '3px', textAlign: 'right' }}><b>Email Id:</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}><a href="#">{this.state.FromEmail}</a></td>
                        </tr>
                        <tr>
                          <td className="mobile-align-left" style={{ border: '1px solid #000', width: '20%', padding: '3px', textAlign: 'right' }}><b>Tracking No.</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}>{this.state.TrackingNumber}</td>
                        </tr>
                        <tr>
                          <td className="mobile-align-left" style={{ border: '1px solid #000', width: '20%', padding: '3px', textAlign: 'right' }}><b>Mode</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}>{this.state.ShipmentType.value}</td>
                        </tr>
                        <tr>
                          <td className="mobile-align-left" style={{ border: '1px solid #000', width: '20%', padding: '3px', textAlign: 'right' }}><b>Sales Rep.</b></td>
                          <td style={{ border: '1px solid #000', width: '30%', padding: '3px' }}>{this.state.DocumentManagedBy}</td>
                        </tr>
                      </table>
                    </td>
                  </tr>
              ...
              <table style={{ width: '100%', marginTop: "10px" }}>
                <tr>
                  <td colspan="2" style={{ fontWeight: 'bold', padding: '3px', border: '1px solid #000', background: '#ffc000' }}>DESCRIPTION OF SERVICES</td>
                  <td style={{ fontWeight: 'bold', padding: '3px', border: '1px solid #000', background: '#ffc000', textAlign: 'right' }}>COST (IN USD)</td>
                </tr>
                {this.renderInvoice()}
                <tr>
                  <td colspan="2" style={{ fontWeight: 'bold', padding: '3px', border: '1px solid #000', background: '#ffc000', textAlign: 'right' }}>TOTAL Cost:</td>
                  <td data-label="Total Cost:" style={{ fontWeight: 'bold', padding: '3px', border: '1px solid #000', background: '#ffc000', textAlign: 'right' }}>$ {parseFloat(this.state.TotalInvoiceCost).toFixed(2)}</td>
                </tr>
              </table>
              <table style={{ width: '100%', marginTop: "10px" }}>
                <tr>
                  <td colspan="2" style={{ border: '1px solid #000', textAlign: 'center', padding: '3px', fontSize: '11px', fontWeight: 'bold', 'color': '#fff', background: '#002060' }}>Best Price <span style={{ borderBottom: '1px solid #fff' }}>Guaranteed</span>. We Will Meet or Beat the Best Door to Door Rates!</td>
                </tr>
              </table>
              <table style={{ width: '100%'}}>
                <tr>
                  <th scope="col" style={{ fontWeight: 'bold', padding: '3px', border: '1px solid #000', background: '#ffc000', width: '50%', textAlign: 'left'}}>Inclusions</th>
                </tr>
                <tr>
                  <td data-label="Inclusions" style={{ width: '50%', border: '1px solid #000' }}>
                    <ul>
                ...
                <tr>
                  <th scope="col" style={{ fontWeight: 'bold', padding: '3px', border: '1px solid #000', background: '#ffc000', width: '50%', textAlign: 'left'}}>Exclusions (If Applicable)</th>
                </tr>
                <tr>
                  <td data-label="Exclusions (If Applicable)" style={{ width: '50%', border: '1px solid #000' }}>
                    <ul>
          ...
          <div className="page-break">
            {this.headerHtml()}
              ...
              <table style={{ width: '100%', marginTop: "10px" }} class="page res-table">
                <tr>
                  <td style={{ border: '1px solid #000', textAlign: 'center', padding: '3px', fontSize: '11px', fontWeight: 'bold', 'color': '#fff', background: '#002060' }}>Insurance Options</td>
                </tr>
              </table>
              <table style={{ width: '100%', fontSize: '12px' }} class="page res-table res-table-right">
                <tr className="mobile-hide">
                  <th scope="col" style={{ fontWeight: 'bold', padding: '3px', border: '1px solid #000', background: '#ffc000', width: '33.33%', textAlign: 'center' }}>Mover’s Liability</th>
                  <th scope="col" style={{ fontWeight: 'bold', padding: '3px', border: '1px solid #000', background: '#ffc000', width: '33.33%', textAlign: 'center' }}>Selective Risk Policy</th>
                  <th scope="col" style={{ fontWeight: 'bold', padding: '3px', border: '1px solid #000', background: '#ffc000', width: '33.33%', textAlign: 'center' }}>All Risk Policy</th>
                </tr>
                <tr>
                  <td data-label="Mover’s Liability" style={{ fontWeight: 'bold', border: '1px solid #000', padding: '3px', textAlign: 'center' }}>$0 Deductible</td>
                  <td data-label="Selective Risk Policy" style={{ fontWeight: 'bold', border: '1px solid #000', padding: '3px', textAlign: 'center' }}>$250 Deductible</td>
                  <td data-label="All Risk Policy" style={{ fontWeight: 'bold', border: '1px solid #000', padding: '3px', textAlign: 'center' }}>$1000 Deductible</td>
                </tr>
                <tr>
                  <td data-label="Mover’s Liability" style={{ border: '1px solid #000', padding: '3px', textAlign: 'center' }}>$0 Deductible</td>
                  <td data-label="Selective Risk Policy" style={{ border: '1px solid #000', padding: '3px', textAlign: 'center' }}>$250 Deductible</td>
                  <td data-label="All Risk Policy" style={{ border: '1px solid #000', padding: '3px', textAlign: 'center' }}>$500 Deductible</td>
                </tr>
                <tr>
                  <td data-label="Mover’s Liability" style={{ border: '1px solid #000', padding: '3px', textAlign: 'center',position:'relative' }}>
                    No premium
                    <div className="flag-label">
                      <span onClick={() => this.scrolldiv(2)}>Sign</span>
                    </div>
                    {this.state.isVisible ? <input id="footer-1.5" maxlength="3" disabled={this.state.hideIntital} onChange={(e)=>this.intialHereChange(e,5)}/> : null}
                    </td>
                  <td data-label="Selective Risk Policy" style={{ border: '1px solid #000', padding: '3px', textAlign: 'center' }}>5.00% premium with<br></br>a $50 minimum</td>
                  <td data-label="All Risk Policy" style={{ border: '1px solid #000', padding: '3px', textAlign: 'center' }}>3.00% premium with<br></br>a $300 minimum</td>
                </tr>
                {/* <tr>
              ...
              <table style={{ width: '100%', marginTop: "10px" , fontSize: '11px' }}>
                <tr>
                  <td><b>Mover’s Liability Explained:</b></td>
                </tr>
                <tr>
                  <td>
                    <p>
                      Movers Liability provides Loss Only coverage of $0.60 Per LBS per article or package.
                      Movers Liability does not provide any coverage on missing or damaged items not mentioned on the packing list.
                  </p>
                  </td>
                </tr>
              </table>
              ...
              <table style={{ width: '100%', fontSize: '11px' }}>
                <tr>
                  <td><b>All Risk Explained:</b></td>
                </tr>
                <tr>
                  <p>
                    Your shipment is covered against risks such as breakage, damage, theft, fire, earthquake, collision, accidents, etc. Shipment packed by owner is not covered under All Risk Insurance. If the shipment is Packed by owner and required repacking at our warehouse; all risk insurance will be activated from SFL Worldwide warehouse once items are inspected and repacked professionally. Shipment will be covered at current value and does not cover shipping charges, packing charges, customs duty etc. SFL Worldwide will take insurance from 3rd party Insurance Company and will not be responsible or liable for any judgment made by the insurance company. It is shipper responsibility to review and accept terms and condition offered by the insurance company or selects any other insurance provider if needed.
              </p>
                </tr>
                  <table style={{ width: '100%', marginTop: "10px", fontSize: '12px' }} class="page res-table">
                  <tr className="mobile-hide">
                    <td colspan="2" style={{ border: '1px solid #000', textAlign: 'center', padding: '3px', fontSize: '11px', fontWeight: 'bold', 'color': '#fff', background: ' #002060', width: '70%' }}>Documents Required at Origin</td>
                    <td rowspan="2" style={{ border: '1px solid #000', textAlign: 'center', padding: '3px', fontSize: '11px', fontWeight: 'bold', 'color': '#fff', background: ' #002060' }}>Documents Required at Destination</td>
                  </tr>
                  <tr className="mobile-hide">
                    <td style={{ fontWeight: 'bold', padding: '3px', border: '1px solid #000', background: '#ffc000' }}>Foreign Passport Holder(s)</td>
                    <td style={{ fontWeight: 'bold', padding: '3px', border: '1px solid #000', background: '#ffc000' }}>Indian Passport Holder(s)</td>
                  </tr>
                  <tr>
                    <td data-label="Documents Required at Origin - Foreign Passport Holder(s)" style={{ border: '1px solid #000', verticalAlign: 'top' }}>
                      <ul style={{fontSize: '11px'}}>
                        <li>Scanned copy of passport photo page only (not the entire passport)</li>
                        <li>Copy of Visa / OCI / PIO card</li>
                        <li>Copy of EIN number / Authorization to us to apply for and obtain EIN on your behalf</li>
                        <li>Signatures on export forms that will be provided by us</li>
                      </ul>
                    </td>
                    <td data-label="Documents Required at Origin - Indian Passport Holder(s)" style={{ border: '1px solid #000', verticalAlign: 'top' }}>
                      <ul style={{fontSize: '11px'}}>
                        <li>Scanned copy of passport photo page (not entire passport)</li>
                        <li>Signatures on export forms that will be provided by us</li>
                      </ul>
                    </td>
                    <td data-label="Documents Required at Destination" style={{ border: '1px solid #000', verticalAlign: 'top' }}>
                      <ul style={{fontSize: '11px' }}>
                ...
                <tr>
                  <td>
                    <b>Shipper's presence in India when the shipment reaches is mandatory. If the shipper has not arrived into India or has to travel out again when the shipment reaches, we cannot clear it and additional charges may incur. We would need Original Passport along with Customer's Signatures on Custom Documentation which will be provided by SFL Worldwide as without this document we won't able to clear shipment from customs and additional charges will incur which SFL Worldwide will not be liable for.</b>
                  </td>
                </tr>
              </table>
            </div>
              ...
              <table style={{ width: '100%', marginTop: "10px" }}>
                ...
                <tr>
                  <td style={{ paddingTop: '20px' }}>
                    <b>Transit Times:</b> SFL Worldwide is only liable for loss of entire package or article that is expressly stated in the household goods descriptive inventory and may occur while your shipment is in our physical possession. Our maximum liability for such loss is limited to an amount not exceeding $0.60 per pound package. SFL Worldwide will not be liable for any damage or missing content inside any package or article. Our liability only covers if entire box/package/article is missing. We are not liable for loss or damage that may occur after the shipment leaves our physical possession.
                  </td>
                </tr>
                <tr>
                  <td style={{ paddingTop: '20px' }}>
          ...
          <div className="page-break">
            ...
            <div className="table-pane">
              <table style={{ width: '100%', marginTop: "10px" }} class="page">
                <tr>
                  <td style={{ border: '1px solid #000', textAlign: 'center', padding: '3px', fontSize: '11px', fontWeight: 'bold', 'color': '#fff', background: ' #002060', width: '50%' }}>Your rights and responsibilities</td>
                </tr>
                <tr>
                ...
                <tr>
                  <td style={{ border: '1px solid #000', fontSize: '11px', textAlign: 'center', fontWeight: 'bold', padding: '15px 5px' }}>
                ...
                <tr>
                  <td style={{ border: '1px solid #000', textAlign: 'center', padding: '3px', fontSize: '11px', fontWeight: 'bold', 'color': '#fff', background: ' #002060', width: '50%' }}>Signature Confirmation <i style={{ fontWeight: 'normal' }}>(Please sign when ready to confirm)</i>:</td>
                </tr>
                <tr>
                  <td style={{position:'relative', height: '80px', border: '1px solid #000', borderWidth: '1px 1px 0 1px', padding: '0 10px' }}>
                    <div className="flag-label">
                      <span>Sign</span>
                    </div>
                    <table className="res-table" style={{ width: '100%' }}>
                      <tr>
                        <td style={{ padding: '5px 0', width: '30%' }} id="footer-3.5">
                          <SignatureCanvas penColor='black'
                            ref={(ref) => { this.sigPad = ref }}
                            canvasProps={{ width: 230, height: 150, className: 'sigCanvas' }} />
                        </td>
                        <td style={{ padding: '5px 0', width: '50%' , verticalAlign: 'bottom' , paddingLeft: '5px' }}>{moment(this.state.currentDate).format(
                          CommonConfig.dateFormat.dateOnly
                        )}</td>
                        <td style={{ padding: '5px 0', textAlign: 'right' , verticalAlign: 'bottom' }}>{this.state.DocumentManagedBy}</td>
                      </tr>
                    </table>
                  </td>
                </tr>
                <tr>
                  <td style={{ border: '1px solid #000', padding: '0 10px', borderWidth: '0 1px 1px 1px' }}>
                    <table style={{ width: '100%', borderTop: '1px solid #000' }}>
                      <tr>
                        <td style={{ padding: '5px 0', width: '37%' }}>Customer Signature</td>
                        <td style={{ padding: '5px 0', width: '40%' }}>Date</td>
                        <td style={{ padding: '5px 0', textAlign: 'right' }}>Working on Proposal</td>
                      </tr>
                      <tr>
                        <td colspan="3" style={{ textAlign: 'center', paddingBottom: '5px' }}>By signing above, you agree that they are incorporated herein by reference and apply to this account.</td>
                      </tr>
                    </table>
                  </td>
                </tr>
          ...
          <div>
            ...
            <div>
              <a href="https://hub.sflworldwide.com/">
                <img src="https://www.sflworldwide.com/wp-content/uploads/2019/05/logo.png" width="185" height="55" />
              </a>
            </div>
          </div>
            <div>
