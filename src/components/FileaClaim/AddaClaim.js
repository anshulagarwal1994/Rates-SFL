...
class AddaClaim extends Component {
    ...
    validate = (event) => {
            ...
            if(CommonConfig.isEmpty(this.state.PersonType)){
                IsFormValid=false;
                this.setState({personTypeErr:true,personTypeHelperText:"Please select person type"});
            }
            if(CommonConfig.isEmpty(this.state.ClaimType)){
                IsFormValid=false;
                this.setState({claimTypeErr:true,claimTypeHelperText:"Please select claim type"});
            }
            if(CommonConfig.isEmpty(this.state.Desiredresolution)){
                IsFormValid=false;
                this.setState({desiredresolutionErr:true,desiredresolutionHelperText:"Please enter desired resolution"});
            } 
            if(CommonConfig.isEmpty(this.state.Comments)){
                IsFormValid=false;
                this.setState({commentsErr:true,commentsHelperText:"Please enter comment"});
            } 
            if(CommonConfig.isEmpty(this.state.TrackingNumber)){
                IsFormValid=false;
                this.setState({trackingnumberErr:true,trackingnumberHelperText:"Please enter tracking number"});
            }
        return IsFormValid
    }
    ...
    handleSubmit = () => {
            ...
            if(this.validate())
            {
                ...
                try {
                        api.post('claim/addClaim', formData).then(res => {
                            if (res.success) {
                                this.hideLoador();
                ...
                cogoToast.error("There were found error in form.Please correct and resubmit form.");
            }
    ...
    handleChange=(event, type) => {
        if (type === "trackingnumber") {
            ...
            if (CommonConfig.isEmpty(trackingnumberVal)) {
                this.setState({ TrackingNumber: trackingnumberVal, trackingnumberErr: true, trackingnumberHelperText: "Please enter Tracking Number" });
            } else {
                this.setState({ TrackingNumber: trackingnumberVal, trackingnumberErr: false, trackingnumberHelperText: "" });
            }
        }
            ...
            if (CommonConfig.isEmpty(commentsVal)) {
                this.setState({ Comments: commentsVal, commentsErr: true, commentsHelperText: "Please e Commnets" });
            } else {
                this.setState({ Comments: commentsVal, commentsErr: false, commentsHelperText: "" });
            }
        }
            ...
            if (CommonConfig.isEmpty(desiredresolutionVal)) {
                this.setState({ Desiredresolution: desiredresolutionVal, desiredresolutionErr: true, desiredresolutionHelperText: "Please enter your Desired Resolution" });
            } else {
                this.setState({ Desiredresolution: desiredresolutionVal, desiredresolutionErr: false, desiredresolutionHelperText: "" });
            }
        }
    }
    ...
    requestChange=(event)=> {
        ...
        if (name === "persontype") {
            ...
            if (CommonConfig.isEmpty(value)) {
                this.setState({ PersonType: value, personTypeErr: true, personTypeHelperText: "Please Select any one option" });
            } else {
                this.setState({ PersonType: value, personTypeErr: false, personTypeHelperText: "" });
            }
        } else if (name === "claimtype") {
            this.setState({ claimTypeCheck: true });
            if (CommonConfig.isEmpty(value)) {
                this.setState({ ClaimType: value, claimTypeErr: true, claimTypeHelperText: "Please Select any one option" });
            } else {
                this.setState({ ClaimType: value, claimTypeErr: false, claimTypeHelperText: "" });
            }
        }
    }
    ...
    deleteSelected = (file) => {
        ...
        this.setState({ fileArray:tempFiles , files:fileTemp , attachmentArray:attachmentTemp });
    }
    ...
    preview=()=>
    {
        ...
        return (
                <div>
                    <span className="img-box-outer">{file.url.includes("blob") ?
                        <img alt="Preview" key={file.preview} src={file.url} /> : <Icon>picture_as_pdf</Icon>}
                        <button className="delete-image" onClick={(event)=>this.deleteSelected(event,file)}>
                            <Icon>clear</Icon>
                        </button>
                    </span>
    ...
    acceptedFiles=(file,event)=>
    {
        ...
        this.setState({attachmentArray:[...this.state.attachmentArray,file[0]]});
        ...
        if(fileType.startsWith("image/"))
        {
            ...
            for (let i = 0; i < this.state.fileObj.length; i++) {
                var obj={url:URL.createObjectURL(this.state.fileObj[i][0]),
    ...
    claimType = () => {
        ...
        return this.state.claimType.map( content => {
                return(
                    <MenuItem  value={content.StringMapKey} > {content.Description}  </MenuItem> 
                )
            })
    }
            ...
            <GridContainer className="UserList-outer">
                ...
                <GridItem xs={12} sm={12} md={12}>
                    <Card>
                        <CardHeader className="btn-right-outer" color="primary" icon>
                            ...
                            {this.state.Access.WriteAccess === 1 ?
                                ...
                                (<Button color="primary" onClick = {() => this.redirectList()}>View All</Button>)
                                 :
                                (null) 
                            }
                        </CardHeader>
                            ...
                            <GridContainer justify="center">
                                <GridItem xs={12} sm={4} md={4}>
                                    <FormControl fullWidth>
                                        <CustomInput
                                            ...
                                            inputProps={{
                                                onFocus: () => this.setState({ trackingnumberCheck: false, trackingnumberErr: false, trackingnumberHelperText: "" }),
                                                onBlur: (event) => this.handleChange(event, "trackingnumber"),
                                                endAdornment: (this.state.trackingnumberCheck !== true ? (<Icon className={styles.User}> local_shipping</Icon>) :
                                                    this.state.trackingnumberErr ? (<InputAdornment position="end">
                                ...
                                <div className="select-spl">
                                    <FormControl fullWidth error={this.state.personTypeErr}>
                                        ...
                                        <FormHelperText>{this.state.personTypeHelperText}</FormHelperText>
                                    </FormControl>
                                </div>
                                </GridItem>
                                ...
                                <GridItem xs={12} sm={4} md={4}>
                                    <div className="select-spl">
                                        <FormControl fullWidth error={this.state.claimTypeErr}>
                                            ...
                                            <FormHelperText>{this.state.claimTypeHelperText}</FormHelperText>
                                        </FormControl>
                                    </div>
                                </GridItem>
                            </GridContainer>
                            ...
                            <GridContainer>
                                <GridItem xs={12} sm={12} md={12}>
                                    <div className="normal-textarea">
                                        ...
                                        <textarea aria-label="minimum height" rowsMin={3} value={this.state.Comments} 
                                         id="comments" onChange={ (event) => this.handleChange(event, "comments")} required/>
                                    </div>
                                </GridItem>
                                <GridItem xs={12} sm={12} md={12}>
                                    <div className="normal-textarea">
                                        <label>Desired Resolution</label>
                                        <textarea aria-label="minimum height" rowsMin={3} value={this.state.Desiredresolution} 
                                        id="desiredresolution"onChange={ (event) => this.handleChange(event, "desiredresolution")} />
                                    </div>
                                </GridItem>
                            </GridContainer>
                            <GridContainer justify="center" className="mt-20">    
                                <GridItem xs={12} sm={12} md={12}>                                
                                    <label className="label-bold">If available please upload any support document(s) or damaged product image(s) to support you’re your claim request. Image size up to 1 MB per image is allowed to be uploaded.</label>
                                </GridItem>
                                <GridItem xs={12} sm={12} md={12}>        
                                    <ReactDropzone
                                        onDropAccepted={(file,event)=>this.acceptedFiles(file,event)}
                                        ...
                                        {({getRootProps, getInputProps}) => (
                                                <section className="dropzone-outer" >
                                                    ...
                                                    <div {...getRootProps()} className="dropzone-inner">
                                                        <input {...getInputProps()} />
                                                        <p> <span>Select File(s)...</span></p>
                                                        <small>Only this extensions are supported .bmp,.gif,.jpeg,.jpg,.tex,.xls,.xlsx,.doc,.docx,.odt,.pdf,.png,.pptx,.ppt,.rtf</small>
                                                    </div>
                                                </section>
                                            )}
                                        </ReactDropzone>
                    ...
                    <div className="shipment-submit">
                        <div className="right">
                            <Button color="rose" onClick={(event) => this.handleSubmit()}>Save</Button>
                        </div>
