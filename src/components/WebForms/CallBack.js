...
class CallBack extends Component{
  ...
  componentDidMount()
  {
    this.setState({AllAccess:CommonConfig.getUserAccess("Call Back").AllAccess});
  ...
  getCallbackData(whereClause){ 
    ...
    try{
        this.showLoador();
        api.post("callback/getCallBackList",data).then(result => {
            if (result.data.success) 
            {
              this.hideLoador();
                if(this.state.AllAccess===1)
                {
                  ...
                  let finalData=result.data.data.filter(x=>x.WorkingOnRequest===this.state.loggedUser);
      ...
      if(value.length !== 0){
        ...
        if(allFilter === -1){
            ...
            for(var j = 0;j < value.length;j++){
              if(j === 0){
                  if(value.length === 1 ){
                      StatusQuery = ` AND ( c.RequestStatus = "`+value[j].value +`")`;
                  }
                  else{
                      StatusQuery = ` AND ( c.RequestStatus = "`+value[j].value +`"`;
                  }
